...
var pushY=0;
    ...
    if(area.rx.toString().startsWith("var x"))area.x+=WORLD.regions.filter(e=>e.file==`regions/${map.name.split(" ").join("-").toLowerCase()}.yaml`)[0].x??0;
    if(area.ry.toString().startsWith("var y"))area.y+=WORLD.regions.filter(e=>e.file==`regions/${map.name.split(" ").join("-").toLowerCase()}.yaml`)[0].y??0;
