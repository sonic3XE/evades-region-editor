...
/*function $d2f179ecccc561fa$export$b9dfb366e63af805(e, t, a, r, c, o, n) {
	const $ = t.image.getImage()
	  , i = t.width
	  , d = t.height
	  , f = Math.floor((-n.x - n.viewportSize.width) / i) * i
	  , s = Math.floor((-n.y - n.viewportSize.height) / d) * d
	  , l = Math.ceil((-n.x + n.viewportSize.width) / i) * i
	  , p = Math.ceil((-n.y + n.viewportSize.height) / d) * d
	  , b = Math.max(a, f)
	  , u = Math.max(r, s)
	  , x = Math.min(a + c, l)
	  , h = Math.min(r + o, p);
	for (let t = b; t < x; t += i)
		for (let f = u; f < h; f += d) {
			let s = Math.min(i, a + c - t)
			  , l = Math.min(d, r + o - f);
			e.drawImage($, 0, 0, s, l, t + n.x, f + n.y, s, l)
		}
}*/
...
function $d2f179ecccc561fa$export$b9dfb366e63af805(e, t, a, r, c, o, n) {
	const $ = t.image.getImage()
	  ...
	  //e.drawImage($, 0, 0, s, l, t + n.x, f + n.y, s, l);
	  return;
	for (let t = a; t < a + c; t += i)
		for (let f = r; f < r + o; f += d) {
			let s = Math.min(i, a + c - t)
			  , l = Math.min(d, r + o - f)
			  , p = t + i / 2
			  , b = f + d / 2;
			const u = Math.abs(p + n.x - n.viewportSize.width/2)
			  , x = Math.abs(b + n.y - n.viewportSize.height/2);
			u > (n.viewportSize.width/2 + s)/camScale || x > (n.viewportSize.height/2 + l)/camScale || e.drawImage($, 0, 0, s, l, t + n.x, f + n.y, s, l)
		}
}
