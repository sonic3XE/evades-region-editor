...
spawnEntities(current_Area)}else{
  ...
  !playtesting&&(ctx.lineWidth=6,ctx.font=`bold 35px tah`,ctx.strokeText(cs,canvas.width/2,20),ctx.fillText(cs,canvas.width/2,20),ctx.font="bold 25px tah",
  ctx.strokeText(`# of zones: ${area.zones.length}`,canvas.width/2,55),ctx.fillText(`# of zones: ${area.zones.length}`,canvas.width/2,55),
  ctx.strokeText(`# of assets: ${area.assets.length}`,canvas.width/2,80),ctx.fillText(`# of assets: ${area.assets.length}`,canvas.width/2,80),
  ctx.strokeText(`# of entities: ${area.entities.length}`,canvas.width/2,105),ctx.fillText(`# of entities: ${area.entities.length}`,canvas.width/2,105));
  ...
  ctx.strokeStyle="#000",ctx.lineWidth=4,ctx.font="bold 20px tah",isForked&&(ctx.textAlign="right",ctx.fillStyle="white",ctx.globalAlpha=0.1,ctx.strokeText("Sonic3XEによって作られた", canvas.width-10, canvas.height-52),ctx.fillText("Sonic3XEによって作られた", canvas.width-10, canvas.height-52),ctx.globalAlpha=1);
